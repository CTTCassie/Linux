...
void udp_server::addrUser(struct sockaddr_in &remote) //添加用户
{
	userList.insert(std::pair<in_addr_t,struct sockaddr_in>(remote.sin_addr.s_addr,remote));
}

void udp_server::delUser(struct sockaddr_in &remote) //删除用户
{
...
int udp_server::recvData(std::string &outString)  //收数据
{
	//可靠性
...
int udp_server::sendData(std::string &inString,struct sockaddr_in &remote)  //发数据
{
